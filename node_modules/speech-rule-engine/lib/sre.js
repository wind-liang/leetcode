...
global&&global.SRE_JSON_PATH&&sre.System.getInstance().setupEngine({mode:sre.Engine.Mode.ASYNC});sre.Cli=function(){this.system=sre.System.getInstance();this.setup={mode:sre.Engine.Mode.SYNC};this.processors=[];this.dp=new sre.SystemExternal.xmldom.DOMParser({errorHandler:function(a,b){throw Error("XML DOM error!");}})};sre.Cli.prototype.set=function(a,b){this.setup[b]="undefined"===typeof a?"semantics"===b?!1:!0:a};sre.Cli.prototype.processor=function(a,b){this.processors.push(b)};
sre.Cli.prototype.enumerate=function(){this.system.setupEngine(this.setup);var a=sre.Engine.getInstance().getAxisValues(),b="",c;for(c in a)b+=c.charAt(0).toUpperCase()+c.slice(1)+" options: "+a[c].slice().sort().join(", ")+"\n";console.log("\n"+b)};sre.Cli.prototype.execute=function(a){var b=sre.SystemExternal.commander;this.runProcessors_(goog.bind(function(a,d){this.system["fileTo"+a](d,b.output)},this),a)};
sre.Cli.prototype.runProcessors_=function(a,b){try{this.processors.length||this.processors.push("Speech"),b&&this.processors.forEach(function(c){a(c,b)})}catch(c){console.log(c.name+": "+c.message),sre.Debugger.getInstance().exit(function(){sre.SystemExternal.process.exit(1)})}};
sre.Cli.prototype.readline=function(){var a=sre.SystemExternal.commander;sre.SystemExternal.process.stdin.setEncoding("utf8");var b=sre.SystemExternal.require("readline").createInterface({input:sre.SystemExternal.process.stdin,output:a.output?sre.SystemExternal.fs.createWriteStream(a.output):sre.SystemExternal.process.stdout}),c="";b.on("line",goog.bind(function(a){c+=a;this.readExpression_(c)&&b.close()},this));b.on("close",goog.bind(function(){this.runProcessors_(goog.bind(function(a,c){b.output.write(this.system["to"+
...
"Locale [code].",c,"locale").option("-s, --semantics","Switch OFF semantics interpretation.",c,"semantics").option("-k, --markup [name]","Generate speech output with markup tags.",c,"markup").option("").option("-p, --speech","Generate speech output (default).",d,"Speech").option("-a, --audit","Generate auditory descriptions (JSON format).",d,"Description").option("-j, --json","Generate JSON of semantic tree.",d,"Json").option("-x, --xml","Generate XML of semantic tree.",d,"Semantic").option("").option("-m, --mathml",
"Generate enriched MathML.",d,"Enriched").option("-g, --generate <depth>","Include generated speech in enriched MathML (with -m option only).",c,"speech").option("-r, --structure","Include structure attribute in enriched MathML (with -m option only).",c,"structure").option("").option("-v, --verbose","Verbose mode.").option("-l, --log [name]","Log file [name].").on("--help",goog.bind(this.enumerate,this)).parse(sre.SystemExternal.process.argv);this.system.setupEngine(this.setup);a.verbose&&sre.Debugger.getInstance().init(a.log);
a.input&&this.execute(a.input);a.args.length?a.args.forEach(goog.bind(this.execute,this)):this.readline();sre.Debugger.getInstance().exit(function(){sre.SystemExternal.process.exit(0)})};sre.SystemExternal.process&&sre.SystemExternal.process.env.SRE_TOP_PATH&&(new sre.Cli).commandLine();
